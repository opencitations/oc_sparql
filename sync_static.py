...
def load_sync_config() -> SyncConfig:
    ...
    try:
        ...
        sync_config = config.get("sync", {"folders": ["static"], "files": []})
...
def scan_changes(
    src_dir: str, dst_dir: str, tracker: ChangeTracker, config: SyncConfig
) -> None:
...
def main():
    ...
    parser.add_argument(
        "--auto", action="store_true", help="run in automatic mode without confirmation"
    )
